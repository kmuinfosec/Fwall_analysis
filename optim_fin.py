...
class topk_delta():
    def __init__(self, path, w, k, d, ugr=False):
        ...
        if self.ugr:  # UGR데이터 셋 사용할때 사용하는 컬럼 윈스에선 사용 안함
            self.columns = ['Rdate', 'duration', 'src_ip', 'dst_ip', 'src_port', 'dst_port', 'proto', 'flag',
                            'forwarding status', 'ToS', 'packets', 'bytes', 'label']
        else:  # 윈스 컬럼
            self.columns = ['Rdate', 'sender_type', 'Ip', 'group_id', 'device_type', 'ckc', 'sensor_id', 'Stime',
                            'Etime', 'src_ip', 'dst_ip', 'Proto', 'src_port', 'dst_port', 'Action', 'signature',
                            'highlight', 'priority', 'Cnt', 'src_country', 'dst_country', 'device_id', 'Node',
                            'category', 'minor', 'Flag', 'msg', 'pool_id', 'pool_group_id', 'cve_code', 'data_type',
                            'is_profile', 'Profile_log_id_list', 'Profile_id', 'Profile_name', 'Ai_flag', 'Opt0',
        ...
        self.from_iso = datetime.datetime.fromisoformat  # 데이트타임 형식으로 바꿔주는 함수
    ...
    def path_window(self):  # 파일경로를 시간대별로 정리 key:value = 시간:경로
        self.w_path = dict()
        for p in self.paths:
            a = self.from_iso(self.fname2time(p))
            ...
            with open(i, 'r', encoding='utf-8') as f:
                while True:
                    ...
                    if not lines:  # 파일 전체 읽으면 반복문 탈출
                        ...
                        for line in lines:
                            # wins 로그가 tsv형식이여서 탭으로 끊고 컬럼이름으로 임시 딕셔너리 만듬
                            tmp = dict(zip(self.columns, line.split('\t')))
    ...
    def k_extract(self):  # top-k의 key값만 가져옴
        ...
        for tables in self.t:  # 전체테이블 반복
            white_list = self.mk_day_set()  # white list를 담을 빈 셋트
    ...
    def run(self):  # 실행
        ...
        for p in l:
            ...
            file_name = f'{start}-{end}'  # 캐시 파일이름
            file_name = reg.sub('', file_name)  # 특수문자 제거
            if not os.path.exists(f'{cache_path + file_name}.pkl'):  # 캐시가 없으면 날짜별로 캐시 저장
                tables = self.count_topk(p)
                with open(f'{cache_path + file_name}.pkl', 'wb') as f:
                    pickle.dump(tables, f)
            else:  # 캐시 존재시 캐시 불러오기
                with open(f'{cache_path + file_name}.pkl', 'rb') as f:
    ...
    def transpose(self, diff):
        # [1일(hh: src_ip, hdh:src_ip, ... ),2일(...), ...] - >[hh:src_ip(1일,2일,3일....), hdh:src_ip(1일,2일,3일...),....]
        ls = []
...
if __name__ == '__main__':
    memory_usage()
    a = topk_delta(r'Wins_FIREWALL\data\202106\FWall', w=24, k=30, d=7)
    a.run()
    memory_usage()
